...
namespace ViennaAdvantage.Model
{
    public class MPayment : X_C_Payment, DocAction, ProcessCall
    {
        ...
        public bool ProcessOnline()
        {
            ...
            try
            {
                VAdvantage.Model.MPayment pay = new VAdvantage.Model.MPayment(GetCtx(), GetC_Payment_ID(), Get_TrxName());
                PaymentProcessor pp = PaymentProcessor.Create(_paymentProcessor, pay);
                if (pp == null)
        ...
        protected override bool BeforeSave(bool newRecord)
        {
                ...
                if (GetC_Order_ID() != 0 || GetC_Invoice_ID() != 0)
                {
                    ...
                    if (GetC_Invoice_ID() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (GetC_Invoice_ID() > 0 && GetC_InvoicePaySchedule_ID() > 0 && newRecord)
                            {
                                string payment = "Select DocumentNo From C_Payment Where C_Invoice_ID=" + GetC_Invoice_ID() + " And C_InvoicePaySchedule_ID=" + GetC_InvoicePaySchedule_ID() + " And Docstatus IN ('DR','IP')";
                                string DocNo = Util.GetValueOfString(DB.ExecuteScalar(payment));
                                if (!string.IsNullOrEmpty(DocNo))
                                {
                                    log.SaveError("Error", Msg.GetMsg(GetCtx(), "INT15_PaymentExist") + DocNo);
                                    return false;
                                }
                            }
                        }
            ...
            if (GetTenderType() == "K")
            {
                ...
                {
                    if (GetCheckDate().Value.Date > DateTime.Now.Date)
                    {
        ...
        public bool TestAllocation()
        {
            ...
            bool IsAdvChargeDoc = false;
            if (Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT COUNT(*) FROM C_PaymentAllocate pa
                                INNER JOIN C_Charge chg ON (pa.C_Charge_ID     =chg.C_Charge_ID)
                                WHERE pa.C_Payment_ID  =" + GetC_Payment_ID() + " AND pa.IsActive ='Y' AND chg.IsAdvanceCharge='Y'", null, Get_TrxName())) > 0)
            {
                IsAdvChargeDoc = true;
            }
            ...
            if (alloc == null)
                alloc = Env.ZERO;
            Decimal total = 0;
            if (!IsINT13_IsMultiCharge())
            {
                total = GetPayAmt();
            }
            else
            {
                total = Util.GetValueOfDecimal(DB.ExecuteScalar("SELECT SUM(amount) from C_PaymentAllocate WHERE C_Payment_ID=" + GetC_Payment_ID() + " AND ISActive='Y' AND NVL( C_Invoice_ID,0) <> 0", null, Get_TrxName()));
            }
            ...
            else
            {
                SetIsAllocated(false);
                SetIsPrepayment(true);
                log.Fine("Allocated= False- Prepayment =True, Case Advance charge document ");
            }
            return change;
        }
        ...
        public void SetC_DocType_ID(bool isReceipt)
        {
            ...
            try
            {
                ...
                if (idr.Read())
                {
                    SetC_DocType_ID(Util.GetValueOfInt(idr[0].ToString()));
                }
                else
                {
        ...
        public void SetC_Invoice_ID(String oldC_Invoice_ID, String newC_Invoice_ID, int windowNo)
        {
            ...
            try
            {
                ...
                if (idr.Read())
                {
                    SetC_BPartner_ID(Util.GetValueOfInt(idr[0].ToString()));
                    int C_Currency_ID = Util.GetValueOfInt(idr[1].ToString());	//	Set Invoice Currency
                    SetC_Currency_ID(C_Currency_ID);
                    //
                    Decimal? invoiceOpen = Util.GetValueOfDecimal(idr[2]);		//	Set Invoice OPen Amount
                    if (invoiceOpen == null)
                        invoiceOpen = Env.ZERO;
                    Decimal? discountAmt = Util.GetValueOfDecimal(idr[3]);		//	Set Discount Amt
                    if (discountAmt == null)
                        discountAmt = Env.ZERO;
                    SetPayAmt(Decimal.Subtract(Util.GetValueOfDecimal(invoiceOpen), Util.GetValueOfDecimal(discountAmt)));
        ...
        public void SetC_Order_ID(String oldC_Order_ID, String newC_Order_ID, int windowNo)
        {
            ...
            try
            {
                ...
                {
                    SetC_BPartner_ID(Util.GetValueOfInt(idr[0].ToString()));
                    int C_Currency_ID = Util.GetValueOfInt(idr[1].ToString());	//	Set Order Currency
                    SetC_Currency_ID(C_Currency_ID);
                    //
                    Decimal? grandTotal = Util.GetValueOfDecimal(idr[2]);		//	Set Pay Amount
                    if (grandTotal == null)
                        grandTotal = Env.ZERO;
                    SetPayAmt(Util.GetValueOfDecimal(grandTotal));
                    SetC_Project_ID(Util.GetValueOfInt(idr[3].ToString()));
                }
                idr.Close();
            }
            catch (Exception e)
        ...
        private void CheckAmt(int windowNo, String columnName)
        {
            ...
            if (C_Invoice_ID != 0)
            {
                ...
                try
                {
                    ...
                    if (idr.Read())
                    {
                        C_Currency_Invoice_ID = Util.GetValueOfInt(idr[1].ToString());
        ...
        public String PrepareIt()
        {
            ...
            // Applied by mohit to check the allocate lines in case of multicharge.
            // Check lines in case multi charge
            if (IsINT13_IsMultiCharge())
            {
                if (Util.GetValueOfInt(DB.ExecuteScalar("SELECT COUNT(*) FROM C_PaymentAllocate WHERE IsActive='Y' AND C_Payment_ID=" + GetC_Payment_ID(), null, Get_TrxName())) == 0)
                {
                    _processMsg = "@NoLinesOnAllocateTab@";
                    return DocActionVariables.STATUS_INVALID;
                }
            }

            #region Check if dimension required for charge the return docaction invalid
            if (GetC_Charge_ID() > 0 && Env.IsModuleInstalled("INT18_"))
            {
                decimal totalAmt = 0;

                string sql = "SELECT Count(C_Charge_ID) FROM C_Charge chg WHERE chg.INT18_IsDimensionbreakupreqd = 'Y' And C_Charge_ID=" + GetC_Charge_ID();
                if (Util.GetValueOfInt(DB.ExecuteScalar(sql)) > 0)
                {
                    string totalSql = "SELECT typ.INT18_Dimensiontype, SUM(amt.INT18_Breakupamount) AS Amt FROM INT18_DimensionBreakup dim INNER JOIN INT18_BreakupType typ ON typ.INT18_Dimensionbreakup_ID = dim.INT18_dimensionbreakup_ID INNER JOIN INT18_BreakupAmt "
                                    + "amt ON amt.INT18_BreakupType_ID = typ.INT18_BreakupType_ID WHERE dim.C_Payment_ID = " + GetC_Payment_ID() + " GROUP BY typ.INT18_DimensionType";
                    DataSet ds = new DataSet();
                    ds = DB.ExecuteDataset(totalSql);
                    MCharge charge = new MCharge(GetCtx(), GetC_Charge_ID(), Get_Trx());
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                        {
                            totalAmt = Util.GetValueOfDecimal(ds.Tables[0].Rows[j]["Amt"]);
                            if (totalAmt < GetPayAmt())
                            {
            ...
            if (!IsReceipt())
            {
                ...
                if (X_C_BPartner.SOCREDITSTATUS_CreditStop.Equals(bp.GetSOCreditStatus()) && bp.GetCreditStatusSettingOn() == "CH")
                ...
                if (X_C_BPartner.SOCREDITSTATUS_CreditHold.Equals(bp.GetSOCreditStatus()) && bp.GetCreditStatusSettingOn() == "CH")
        ...
        public String CompleteIt()
        {
            ...
            else if (Env.IsModuleInstalled("VA009_"))
            {
                ...
                {
                    DB.ExecuteQuery(@"Update VA009_OrderPaySchedule set VA009_ISPAID='N', C_Payment_ID=null, 
        ...
        private MPayment CreateCounterDoc()
        {
            ...
            int C_BankAccount_ID = DB.GetSQLValue(Get_Trx(), sql);
        ...
        private bool AllocateInvoice()
        {
            try
            {
                ...
                DateTime? _dtTrx = null;
                MInvoice invoice = new MInvoice(GetCtx(), GetC_Invoice_ID(), Get_TrxName());
                if (invoice.GetDateAcct() > GetDateAcct())
                {
                    _dtTrx = invoice.GetDateAcct();
                }
                else
                {
                    _dtTrx = GetDateAcct();
        ...
        private Boolean AllocatePaySelection()
        {
            ...
            try
            {
                ...
                while (idr.Read())
                {
                    int C_BPartner_ID = Util.GetValueOfInt(idr[0].ToString());
                    int C_Invoice_ID = Util.GetValueOfInt(idr[1].ToString());
                    if (C_BPartner_ID == 0 && C_Invoice_ID == 0)
                        continue;
                    Boolean isSOTrx = "Y".Equals(idr[2].ToString());
                    Decimal payAmt = Util.GetValueOfDecimal(idr[3]);
                    Decimal discountAmt = Util.GetValueOfDecimal(idr[4]);
                    Decimal writeOffAmt = Util.GetValueOfDecimal(idr[5]);
                    Decimal openAmt = Util.GetValueOfDecimal(idr[6]);
                    Decimal overUnderAmt = 0;
                    if (Env.IsModuleInstalled("VA009_"))
                    {
                        if (Util.GetValueOfDecimal(idr[5]) < 0)
                        {
                            overUnderAmt = Util.GetValueOfDecimal(idr[5]);
                            writeOffAmt = decimal.Zero;
                        }
                        else
                        {
                            writeOffAmt = Util.GetValueOfDecimal(idr[5]);
                    ...
                    if (Env.IsModuleInstalled("VA009_"))
                    {
                        aLine.SetC_InvoicePaySchedule_ID(Util.GetValueOfInt(idr[7]));
                    }
                    aLine.SetC_Payment_ID(GetC_Payment_ID());
        ...
        private void DeAllocate()
        {
            ...
            if (GetC_Invoice_ID() != 0)
            {
                ...
                sql = "UPDATE C_Order o "
                    ...
                    + " AND C_Payment_ID=" + GetC_Payment_ID();
                no = DB.ExecuteQuery(sql, null, Get_Trx());
                if (no != 0)
                {
        ...
        public Boolean VoidIt()
        {
            ...
            MPayment mInvoice = new MPayment(GetCtx(), GetC_Payment_ID(), Get_TrxName());

            MAllocationLine allocationLine = new MAllocationLine(GetCtx(), mInvoice.GetC_Payment_ID(), Get_TrxName());
            string sql2 = @"select count(C_AllocationLine_ID) from C_AllocationLine where  c_payment_id= " + GetC_Payment_ID();
            int id = DB.GetSQLValue(Get_Trx(), sql2);
            if (id > 0)
            {
        ...
        public Boolean ReverseCorrectIt()
        {
            ...
            string sql3 = @"select count(C_Payment_ID) from C_Payment  where  C_Payment_ID= " + GetC_Payment_ID();
            int id1 = DB.GetSQLValue(Get_Trx(), sql3);
            if (id1 > 0)
            {
                _processMsg = "@Already Reconciled cannot Reverse or Void the transaction@";
                return false;
            }
            ...
            if (Env.IsModuleInstalled("VA009_"))
            {
                ...
                string sql = "SELECT UNIQUE C_PaymentAllocate_ID FROM C_PaymentAllocate WHERE IsActive = 'Y' AND C_Payment_ID =  " + GetC_Payment_ID();
                DataSet ds = new DataSet();
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        reversalPaymentAllocate.SetINT13_AllocateType(originalPaymentAllocate.GetINT13_AllocateType());
                        ...
                        else if (originalPaymentAllocate.GetC_Charge_ID() > 0)
                        {
                            reversalPaymentAllocate.SetC_Charge_ID(originalPaymentAllocate.GetC_Charge_ID());
                            reversalPaymentAllocate.SetC_Tax_ID(originalPaymentAllocate.GetC_Tax_ID());
                            reversalPaymentAllocate.SetTaxAmount(originalPaymentAllocate.GetTaxAmount());
                        }
