...
namespace ViennaAdvantage.Model
{
    ...
    public class MOrder : X_C_Order, DocAction
    {
        ...
        private static int counterOrgId = 0;
        private static int counterWarehouseId = 0;
        private static MBPartner counterBPartner = null;
         ...
         *	@return Order
         */
        public static MOrder CopyFrom(MOrder from, DateTime? dateDoc, int C_DocTypeTarget_ID, bool counter, bool copyASI, Trx trxName, bool fromCreateSO = false)
        {
            ...
            if (counter)
            {
                ...
                // set counter BP Org
                if (counterOrgId > 0)
                    to.SetAD_Org_ID(counterOrgId);

                //set warehouse
                if (counterWarehouseId > 0)
                    to.SetM_Warehouse_ID(counterWarehouseId);

                // set Counter BP Details
                if (counterBPartner != null)
                    to.SetBPartner(counterBPartner);
                ...
                if (counterBPartner != null && ((to.IsSOTrx() && !pl.IsSOPriceList()) || (!to.IsSOTrx() && pl.IsSOPriceList())))
                {
            ...
            if (to.CopyLinesFrom(from, counter, copyASI, fromCreateSO) == 0)
            {
                ValueNamePair pp = VLogger.RetrieveError();
                if (pp != null)
                {
        ...
        */
        public int CopyLinesFrom(MOrder otherOrder, bool counter, bool copyASI, bool fromCreateSO = false)
        {
            int count = 0;
            try
            {
        ...
        */
        public new void SetProcessed(bool processed)
        {
            ...
            int noSech = DB.ExecuteQuery("UPDATE SYR01_DeliverySchedule SET Processed = '" + (processed ? "Y" : "N") +
                "' WHERE C_OrderLine_ID IN (SELECT C_OrderLine_ID FROM C_OrderLine WHERE C_Order_ID=" + GetC_Order_ID() + ")", null, Get_TrxName());
            _lines = null;
            _taxes = null;
            log.Fine(processed + " - Lines=" + noLine + ", Tax=" + noTax + ", Schedule=" + noSech);
        }
        ...
        */
        protected override bool BeforeSave(bool newRecord)
        {
            try
            {
                ...
                if (GetM_PriceList_ID() == 0)
                {
                    ...
                    int ii = Util.GetValueOfInt(DB.ExecuteScalar("SELECT M_PriceList_ID FROM M_PriceList "
                ...
                if (GetC_Currency_ID() == 0)
                {
                    ...
                    int ii = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, null));
                    if (ii != 0)
                        SetC_Currency_ID(ii);
                    else
                ...
                if (GetC_PaymentTerm_ID() == 0)
                {
                    ...
                    else
                    {
                        ...
                        ii = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, null));
                        if (ii != 0)
                            SetC_PaymentTerm_ID(ii);
                    }
                }
                ...
                if (dt.GetDocBaseType() == "BOO") ///dt.GetValue() == "BSO" || dt.GetValue() == "BPO")
                {
                    ...
                    if (GetDateOrdered() != null && GetDatePromised() != null)
                    {
                        ...
                        {
                            log.SaveError(Msg.Translate(GetCtx(), "VIS_OrderDateGrtrThanPromisedDate"), "");
                            return false;
                        }
                    }
                }
        ...
        public String PrepareIt()
        {
            ...
            #region Check if dimension required for charge then return docaction invalid
            if (Env.IsModuleInstalled("INT18_"))
            {
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    {
                        string sqlcount = "SELECT Count(il.C_Charge_ID) FROM C_OrderLine il INNER JOIN C_Charge chg ON chg.C_Charge_id = il.C_Charge_ID WHERE chg.INT18_IsDimensionbreakupreqd = 'Y' And il.C_OrderLine_ID=" + line.GetC_OrderLine_ID();
                        if (Util.GetValueOfInt(DB.ExecuteScalar(sqlcount)) > 0)
                        {
                            string totalSql = "SELECT typ.INT18_Dimensiontype, SUM(amt.INT18_Breakupamount) AS Amt FROM INT18_DimensionBreakup dim INNER JOIN INT18_BreakupType typ ON typ.INT18_Dimensionbreakup_ID = dim.INT18_dimensionbreakup_ID INNER JOIN INT18_BreakupAmt "
                                            + "amt ON amt.INT18_BreakupType_ID = typ.INT18_BreakupType_ID WHERE dim.C_OrderLine_ID = " + line.GetC_OrderLine_ID() + " or dim.INT18_PurchaseOrderLine=" + line.GetC_OrderLine_ID() + " GROUP BY typ.INT18_DimensionType";
        ...
        private bool ReserveStock(MDocType dt, MOrderLine[] lines)
        {
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    VAdvantage.Model.MProduct product1 = line.GetProduct();
                    if (product1 != null)
                    {
        ...
        private MInOut CreateShipment(MDocType dt, DateTime? movementDate)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < oLines.Length; i++)
                {
                    ...
                    OnHandQty = Convert.ToDecimal(DB.ExecuteScalar(sql));
                    if (wh.IsDisallowNegativeInv() == true)
                    {
                        if (oLine.GetQtyEntered() > QtyAvail)
                        {
                            if (DocSubTypeSO == "WR" || DocSubTypeSO == "WP")
                            {
                                //if (QtyAvail < 0)
                                //{
                                // _processMsg = "Shipment Could Not Complete Due To Insufficent Qty, Available Qty Is " + (QtyAvail + oLine.GetQtyEntered());
                                ...
                                {
                                    VAdvantage.Model.MProduct product = ioLine.GetProduct();
                    ...
                    {
                        //_processMsg = "Shipment Could Not Complete Due To Insufficent Qty, Available Qty Is " + QtyAvail;
