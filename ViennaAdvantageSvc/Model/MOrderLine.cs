...
namespace ViennaAdvantage.Model
{
    ...
    public class MOrderLine : X_C_OrderLine
    {
        ...
        public void SetLineNetAmt()
        {
            ...
            if (Util.GetValueOfInt(DB.ExecuteScalar("SELECT COUNT(AD_MODULEINFO_ID) FROM AD_MODULEINFO WHERE PREFIX='ED007_'")) > 0)
            {
                                    ...
                                    if (dsDiscountCombination.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        if (BPAndProductValue > 0)
                                        {
                                                 ...
                                                 && Util.GetValueOfDecimal(GetLineNetAmt()).CompareTo(Decimal.Round(Decimal.Multiply(GetPriceLimit(), GetQtyEntered()), precision)) < 0)
                                            {
                                                ...
                                                if (GetPriceList() != 0)
                                                {
                                                    Decimal Discount = Util.GetValueOfDecimal(((Decimal.ToDouble(GetPriceList()) - Decimal.ToDouble(GetPriceLimit())) / Decimal.ToDouble(GetPriceList()) * 100.0));
        ...
        public int GetPrecision()
        {
            ...
            int i = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, Get_TrxName()));
            _precision = i;
            return (int)_precision;
        }
        ...
        public VAdvantage.Model.MProduct GetProduct()
        {
            if (_product == null && GetM_Product_ID() != 0)
                _product = VAdvantage.Model.MProduct.Get(GetCtx(), GetM_Product_ID());
            return _product;
        }
        ...
        public void SetS_ResourceAssignment_ID(String oldS_ResourceAssignment_ID, String newS_ResourceAssignment_ID, int windowNo)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    M_Product_ID = Util.GetValueOfInt(dr[0].ToString());//.getInt (1);
                    Name = dr[1].ToString();//.getString(2);
                    Description = dr[2].ToString();//.getString(3);
                    Qty = Util.GetValueOfDecimal(dr[3].ToString());//.getBigDecimal(4);
                }
            }
            catch (Exception e)
            {
        ...
        public bool IsTaxIncluded()
        {
            if (_M_PriceList_ID == 0)
            {
                _M_PriceList_ID = Util.GetValueOfInt(DB.ExecuteScalar("SELECT M_PriceList_ID FROM C_Order WHERE C_Order_ID=" + GetC_Order_ID(), null, Get_TrxName()));
            }
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            ...
            {
                log.SaveError(Msg.Translate(GetCtx(), "VIS_NOProductOrCharge"), "");
                return false;
            }

            Decimal? PriceActual, PriceEntered;
            Decimal? QtyOrdered, QtyEntered;
            ...
            //Code add by Anuj
            //Shipment/partial Shipment done but after reactive SO .Qty will be not updated.(task given by Yadveer(01-Mar-2017))  
            #region
            if (!newRecord && !Is_ValueChanged("QtyDelivered") && !Is_ValueChanged("QtyInvoiced") && Ord.IsSOTrx() && Ord.IsReturnTrx() && GetC_OrderLine_ID() != 0) //added by arpit on 12 April,2017 Is_ValueChanged("QtyInvoiced")
            {
                int _count = Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT Count(*) FROM m_inoutLine ML Inner JOIN M_inout M on m.m_inout_id =ml.m_inout_id 
                             WHERE ml.IsActive = 'Y' AND m.DocStatus NOT IN ('RE' , 'VO') AND M.issotrx ='Y' 
                             AND m.isreturntrx='N' AND ML.c_orderline_id =" + GetC_OrderLine_ID(), null, Get_Trx()));
                if (_count > 0)
                {
                    log.SaveWarning("Warning", Msg.GetMsg(GetCtx(), "GOM01_ShipmentDonenotabletochangeQty"));
                    return false;
                }
            }
            #endregion
            ...
            if (isReturnTrx)
            {
                ...
                {
                    log.SaveError(Msg.Translate(GetCtx(), "VIS_ReturnQtyMoreThnOrder"), "");
                    return false;
                }
