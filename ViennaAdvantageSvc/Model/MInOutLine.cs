...
namespace ViennaAdvantage.Model
{
     ...
     */
        public void SetOrderLine(MOrderLine oLine, int M_Locator_ID, Decimal Qty)
        {
            ...
            VAdvantage.Model.MProduct product = oLine.GetProduct();
            if (product == null)
            {
                SetM_Product_ID(0);
                SetM_AttributeSetInstance_ID(0);
       ...
       */
        ...
        */
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            ...
            MInOut inO = new MInOut(GetCtx(), GetM_InOut_ID(), Get_Trx());
            ...
            MProduct MProduct = new MProduct(GetCtx(), GetM_Product_ID(), null);
            ...
            // if (wh.IsDisallowNegativeInv() == true && (!(inO.GetDocStatus() == "CO" || inO.GetDocStatus() == "CL")))
            MProduct mProduct = new MProduct(GetCtx(), GetM_Product_ID(), Get_Trx());
            if (mProduct.GetProductType() == X_M_Product.PRODUCTTYPE_Item)
            {
                if (wh.IsDisallowNegativeInv() == true && (!(inO.GetDocStatus() == "CO" || inO.GetDocStatus() == "CL"
                                    || inO.GetDocStatus() == "RE" || inO.GetDocStatus() == "VO")))
                {
            ...
            if (!IsReveresed)
            {
                // Check to allow the multiple shipments of service type product- Change by mohit- 10/29/2018
                if (_Product.GetProductType() != "S")
                {
        ...
        private bool CheckProductExist(bool futureDate)
        {
            MInOut InOut = new MInOut(GetCtx(), GetM_InOut_ID(), Get_TrxName());
            string Sql = "Select Count(M_InOutLine_ID) From M_InOut io Inner Join m_inoutline il on io.m_inout_id=il.m_inout_id Where io.IsSotrx='" + InOut.IsSOTrx() + "' AND io.IsReturnTrx='" + InOut.IsReturnTrx() + "' AND io.AD_Client_ID=" + GetAD_Client_ID() + " AND "
                        + "io.AD_Org_ID=" + GetAD_Org_ID() + " AND il.M_Product_ID= " + GetM_Product_ID();
            if (futureDate)
            {
