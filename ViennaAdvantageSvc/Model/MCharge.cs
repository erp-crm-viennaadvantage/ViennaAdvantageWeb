    ...
    public class MCharge : X_C_Charge
    {
        ...
        public static MAccount GetAccount(int C_Charge_ID, MAcctSchema aSchema, Decimal amount)
        {
            ...
            try
            {
                ...
                if (dr.Read())
                    Account_ID = Util.GetValueOfInt(dr[acct_index - 1].ToString());
                dr.Close();
                //pstmt.close();
            }
        ...
        {
            if (Util.GetValueOfInt(DB.ExecuteScalar("SELECT COUNT(*) FROM ad_moduleinfo WHERE prefix='GMT01_'")) > 0)
            {
                if (GetDTD001_ChargeType() == "BRC" || GetDTD001_ChargeType() == "CRC" || GetDTD001_ChargeType() == "SRC")
                {
                    if (newRecord)
                    {
                        if (Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT COUNT(*) FROM C_Charge WHERE isactive ='Y' AND AD_Org_ID =" + GetAD_Org_ID() + @"
                                AND AD_Client_ID     = " + GetAD_Client_ID() + @" AND DTD001_ChargeType='" + GetDTD001_ChargeType() + "'")) > 0)
                        {
                            log.SaveError(Msg.GetMsg(GetCtx(), "GMT01_ChargeExist"), "");
                            return false;
                        }
                    }
                    else
                    {
                        if (Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT COUNT(*) FROM C_Charge WHERE isactive ='Y' AND AD_Org_ID =" + GetAD_Org_ID() + @"
                                AND AD_Client_ID     = " + GetAD_Client_ID() + @" AND DTD001_ChargeType='" + GetDTD001_ChargeType() + "' AND C_Charge_ID!=" + GetC_Charge_ID() + "")) > 0)
                        {
                            log.SaveError(Msg.GetMsg(GetCtx(), "GMT01_ChargeExist"), "");
                            return false;
                        }
