    ...
    public class MInventoryLine : X_M_InventoryLine
    {
        ...
        private void SetQtyBook(int windowNo, String columnName)
        {
            ...
            try
            {
                ...
                if (idr.Read())
                {
                    qtyBook = Util.GetValueOfDecimal(idr[0]);
                }
                idr.Close();
            }
            catch (Exception e)
            {
                ...
                //ErrorLog.FillErrorLog("MInventoryLine.SetQtyBook", GlobalVariable.LAST_EXECUTED_QUERY, e.Message, VAdvantage.Framework.Message.MessageType.ERROR);
            }
            SetQtyBook(qtyBook);
            //
