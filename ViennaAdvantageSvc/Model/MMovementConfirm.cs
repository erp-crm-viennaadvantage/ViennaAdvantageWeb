...
namespace ViennaAdvantage.Model
{
    ...
    public class MMovementConfirm : X_M_MovementConfirm, DocAction
    {
        ...
        private VAdvantage.Model.MInventory _inventoryFrom = null;
        /**	Physical Inventory To	*/
        private VAdvantage.Model.MInventory _inventoryTo = null;
        ...
        public String CompleteIt()
        {
            ...
            if (Env.HasModulePrefix("DTD001_", out mInfo))
            {
                ...
                VAdvantage.Model.MRequisitionLine requisitionLine = null;
                VAdvantage.Model.MStorage storage = null;
                for (int i = 0; i < lines.Length; i++)
                {
                    MMovementLineConfirm confirm = lines[i];
                    if (confirm.GetDifferenceQty() > 0)
                    {
                        ...
                        {
                            requisitionLine = new VAdvantage.Model.MRequisitionLine(GetCtx(), movementLine.GetM_RequisitionLine_ID(), Get_Trx());
                            ...
                            storage = VAdvantage.Model.MStorage.Get(GetCtx(), movementLine.GetM_Locator_ID(), movementLine.GetM_Product_ID(), movementLine.GetM_AttributeSetInstance_ID(), Get_Trx());
                            if (storage == null)
                            {
                                storage = VAdvantage.Model.MStorage.Get(GetCtx(), movementLine.GetM_Locator_ID(), movementLine.GetM_Product_ID(), 0, Get_Trx());
                            }
