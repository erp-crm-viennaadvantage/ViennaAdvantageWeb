    ...
    public class MInvoiceLine : X_C_InvoiceLine
    {
        ...
        */
        public static MInvoiceLine GetOfInOutLine(MInOutLine sLine)
        {
            ...
            try
            {
                ...
                try
                {
                    ds = DB.ExecuteDataset(sql, null, sLine.Get_TrxName());
        ...
        */
        ...
        public Boolean IsTaxIncluded()
        {
            ...
            {
                _M_PriceList_ID = DB.GetSQLValue(Get_TrxName(),
            ...
            String newS_ResourceAssignment_ID, int windowNo)
        {
            ...
            try
            {
                ...
                if (idr.Read())
                {
                    M_Product_ID = Util.GetValueOfInt(idr[0]);
                    Name = idr.GetString(1);
                    Description = idr.GetString(2);
                    Qty = Util.GetValueOfDecimal(idr[3]);
                }
                idr.Close();
        ...
        protected override bool BeforeSave(bool newRecord)
        {
            ...
            try
            {
                ...
                {
                    DataSet dsOrder = DB.ExecuteDataset("SELECT PriceEntered , QtyEntered FROM C_ORDERLINE WHERE IsActive = 'Y' AND C_OrderLine_ID = " + GetC_OrderLine_ID());
                    if (dsOrder != null && dsOrder.Tables.Count > 0 && dsOrder.Tables[0].Rows.Count > 0)
                    {
                        //if (Util.GetValueOfDecimal(dsOrder.Tables[0].Rows[0]["PriceEntered"]) < GetPriceEntered())
                        //{
                        //    log.SaveError("Error", Msg.GetMsg(GetCtx(), "GOM01_AmountCantbeGreater"));
                        //    return false;
                        //}
                        if (Util.GetValueOfDecimal(dsOrder.Tables[0].Rows[0]["QtyEntered"]) < GetQtyEntered())
                        {
                            log.SaveError("Error", Msg.GetMsg(GetCtx(), "GOM01_QtyCantbeGreater"));
                            return false;
                        }
                    }
                }
                ...
                if (!inv.IsSOTrx())
                {
                    ...
                    {
                        if (Util.GetValueOfBool(Get_Value("INT16_IsAssetRelated")) && GetA_Asset_ID() > 0 && inv.IsSOTrx() && !inv.IsReturnTrx())
                        {
                            MAsset asset = new MAsset(GetCtx(), GetA_Asset_ID(), Get_TrxName());
                            if (GetQtyInvoiced() > asset.GetQty())
                            {
                                log.SaveError("Error", Msg.GetMsg(GetCtx(), "INT16_QtygrtrthanAsset"));
                                return false;
                            }
                        }
                    }
        ...
        public String AllocateLandedCosts()
        {
            try
            {
                ...
                if (lcs.Length == 0)
                    return "";
                //Added By Manjot For Cost Sheet
