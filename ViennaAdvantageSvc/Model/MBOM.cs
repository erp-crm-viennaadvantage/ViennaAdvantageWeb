        ...
        */
            ...
            Trx trxName, String whereClause)
        {
            ...
            try
            {
                ...
                idr = VAdvantage.DataBase.DB.ExecuteReader(sql, null, trxName);
                dt = new DataTable();
                dt.Load(idr);
                idr.Close();
        ...
        {
            //	BOM Type
            int _CountGOM01 = Util.GetValueOfInt(DB.ExecuteScalar("SELECT COUNT(AD_MODULEINFO_ID) FROM AD_MODULEINFO WHERE PREFIX='GOM01_'  AND IsActive = 'Y'"));
            if (newRecord || Is_ValueChanged("BOMType"))
            {
                ...
                {
                    int count = Util.GetValueOfInt(DB.ExecuteScalar("Select Count(*) From M_BOM Where GOM01_Priority =" + GetGOM01_Priority() + " AND BOMUse='" + GetBOMUse() + "' AND M_Product_ID =" + GetM_Product_ID()));
                    if (count > 0)
                    {
                        log.SaveError("Error", Msg.ParseTranslation(GetCtx(),
                              "Can only have unique priority"));
                        return false;
                    }
                }
            }
            else
            {
                if (_CountGOM01 > 0)
                {
                    if (newRecord || Is_ValueChanged("GOM01_Priority"))
                    {
                        int count = Util.GetValueOfInt(DB.ExecuteScalar("Select Count(*) From M_BOM Where GOM01_Priority =" + GetGOM01_Priority() + " AND BOMUse='" + GetBOMUse() + "' AND M_Product_ID =" + GetM_Product_ID()));
                        if (count > 0)
                        {
