...
namespace ViennaAdvantage.Model
{
	 ...
	 */
        ...
        private bool CheckInventoryExist(bool futureDate)
        {
            ...
            string Sql = @"Select Count(M_Inventory_ID) From M_Inventory Inv Inner Join C_Doctype doc on doc.C_Doctype_ID=Inv.C_Doctype_ID Where doc.IsInternalUse='" + IsnternalUse + @"' AND 
                                 doc.IsDecanting= '" + Decanting + @"'   AND inv.AD_Client_ID=" + GetAD_Client_ID() + " AND inv.AD_Org_ID=" + GetAD_Org_ID() + " ";
            if (futureDate)
            {
                Sql += " AND inv.DocStatus IN ('CO','CL','IP','DR') AND inv.MovementDate > " + GlobalVariable.TO_DATE(GetMovementDate(), true);
            }
            else
            {
                Sql += " AND inv.DocStatus IN ('IP') AND inv.MovementDate < " + GlobalVariable.TO_DATE(GetMovementDate(), true);
            }
        ...
        private bool Checkmovdate(bool futureDate)
        {
            ...
            string IsnternalUse = Util.GetValueOfBool(Doctype.Get_Value("IsInternalUse")) ? "Y" : "N";
            string Sql = @"Select Count(M_Inventory_ID) From M_Inventory Inv Inner Join C_Doctype doc on doc.C_Doctype_ID=Inv.C_Doctype_ID Where doc.IsInternalUse='" + IsnternalUse + @"' AND 
                                 doc.IsDecanting= '" + Decanting + @"'   AND inv.AD_Client_ID=" + GetAD_Client_ID() + " AND inv.AD_Org_ID=" + GetAD_Org_ID() + "AND inv.DocStatus IN('IP') AND inv.MovementDate < sysdate AND inv.M_Inventory_ID =" + GetM_Inventory_ID() ;
        ...
        public String PrepareIt()
        {
            ...
            //	TODO: Add up Amounts
            //	setApprovalAmt();
            MInventoryLine[] Invlines = GetLines(false);
            log.Info("total Lines=" + Invlines.Count());
            //
            log.Info("Lines Loop Started");
            for (int i = 0; i < Invlines.Length; i++)
            {
                MInventoryLine line = Invlines[i];
                MProduct Product = new MProduct(GetCtx(), line.GetM_Product_ID(), Get_TrxName());
                string query = "SELECT NVL(pp.pricestd,0) as PriceStd  "
                                ...
                                + " WHERE pp.pricestd > 0 AND pl.isactive = 'Y' and plv.isactive = 'Y' and pp.isactive = 'Y' and pp.m_product_id = " + line.GetM_Product_ID()
                                + " AND NVL(pp.m_attributesetinstance_id, 0) = " + line.GetM_AttributeSetInstance_ID()
                                + " AND pp.c_uom_id = " + Product.GetC_UOM_ID() + " AND pl.issopricelist = 'N' ORDER BY pl.m_pricelist_id asc";
                decimal StdPrice = Util.GetValueOfDecimal(DB.ExecuteScalar(query));
                if (StdPrice <= 0)
                {
                    _processMsg = "@NoPriceListDefined @" + Product.GetName();
                    return DocActionVariables.STATUS_INVALID;
                }
            }
        ...
        public String CompleteIt()
        {
            ...
            {
                MInventoryLine line = lines[i];
                VAdvantage.Model.MInventoryLine InvLine1 = new VAdvantage.Model.MInventoryLine(GetCtx(), line.GetM_InventoryLine_ID(), Get_TrxName());
                if (!line.IsActive())
                    continue;
                if (val == "Y")
                {
                ...
                if (client.IsCostImmediate())
                {
                    ...
                    VAdvantage.Model.MProduct Prod = new VAdvantage.Model.MProduct(GetCtx(), product1.GetM_Product_ID(), Get_TrxName());
                    if (product1.GetProductType() == "I") // for Item Type product
                    {
                        ...
                        if (!IsInternalUse()) // Physical Inventory
                        {
                            ...
                            if (!MCostQueue.CreateProductCostsDetails(GetCtx(), GetAD_Client_ID(), GetAD_Org_ID(), Prod, line.GetM_AttributeSetInstance_ID(),
                           "Physical Inventory", InvLine1, null, null, null, null, 0, quantity, Get_TrxName(), out conversionNotFoundInOut, optionalstr: "window"))
                            {
                                ...
                                _processMsg = "Could not create Product Costs1";
                                //return DocActionVariables.STATUS_INVALID;
                            }
                            else
                            {
                        ...
                        else // Internal Use Inventory
                        {
                            ...
                            if (!MCostQueue.CreateProductCostsDetails(GetCtx(), GetAD_Client_ID(), GetAD_Org_ID(), Prod, line.GetM_AttributeSetInstance_ID(),
                           "Internal Use Inventory", InvLine1, null, null, null, null, 0, quantity, Get_TrxName(), out conversionNotFoundInOut, optionalstr: "window"))
                            {
                                if (!conversionNotFoundInventory1.Contains(conversionNotFoundInventory))
                                {
                                    conversionNotFoundInventory1 += conversionNotFoundInventory + " , ";
                                }
                                _processMsg = "Could not create Product Costs2";
                                // return DocActionVariables.STATUS_INVALID;
                            }
                            else
                            {
           ...
           int Org_ID, MCostElement ce, decimal movementQty)
        {
            ...
            VAdvantage.Model.MProduct prod = new VAdvantage.Model.MProduct(GetCtx(), product.GetM_Product_ID(), Get_TrxName());
            MCostQueue[] cQueue = MCostQueue.GetQueue(prod, M_ASI_ID, mas, Org_ID, ce, null);
        ...
        private void CheckMaterialPolicy()
        {
            ...
            {
                MInventoryLine line = lines[i];
                VAdvantage.Model.MInventoryLine InvLine = new VAdvantage.Model.MInventoryLine(GetCtx(), line.GetM_InventoryLine_ID(), Get_TrxName());
                bool needSave = false;
                        ...
                        for (int ii = 0; ii < storages.Length; ii++)
                        {
                            ...
                            if (ii == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    MInventoryLineMA ma = new MInventoryLineMA(InvLine,
